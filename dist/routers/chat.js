...
router.get('/getMessagesByChatID/:id', authenticateJWT, async (req, res) => {
...
router.get('/getChatTitleByID/:id', authenticateJWT, async (req, res) => {
    const id = req.params.id;
    try {
...
router.delete('/deleteAllChatsByUserID', authenticateJWT, async (req, res) => {
    const { id } = req.user;
    try {
        const conversations = await Conversation.find({ userID: id });
